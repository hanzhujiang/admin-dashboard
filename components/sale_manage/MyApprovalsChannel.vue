<template>
  <div class="wrapper clearfix">
    ...
    <div class="wrapper-header clearfix">
      <div class="title">
        <span >{{rs.my_approvals_channel}}</span>
      </div>
    </div>
                        ...
                        <template slot-scope="scope">
                          ...
                          <el-popover
                            ...
                            <div style="min-height:80px;width:300px">
                              <p v-for="(d,index) in reasonlist" :key="index">({{index+1}})&nbsp;&nbsp;{{d.content}}&nbsp;</p>
                            </div>
...
import { getAgencyApproved, getReject } from '@/api/saleApi.js'
export default{
  data() {
    return {
      total:0,
  ...
  methods:{
    ...
    getRequest(data){
      getAgencyApproved(data).then(res=>{
        if(res.data.success){
