  ...
  <div>
    <form autocomplete="on">
      <div class="wrapper-condition clearfix">
        <el-row>
          <el-col :span="12">
            <!-- <el-checkbox style="float:left;" v-model="statusCheck">{{rs.only_show_pending_settle}}</el-checkbox> -->
          </el-col>
          <el-col :span="24">
            <div class="filter-search fr" style="width:30%;">
              <el-input :placeholder="rs.search_orders" v-model="getData.condition" 
                  class="input-key" size='mini' @change="search" @keyup.enter='search' clearable>
...
export default {
  ...
  watch:{
    statusCheck(val){
      if(val){
        this.getData.salestatus ='40'
      }else{
            this.getData.salestatus ='40,30'
      }
      this.Property(this.getData);
    }
  },
  methods: {
    ...
    goRouter(path,row){
      ...
      sessionStorage.setItem('isFalg',JSON.stringify({saleid:row.saleid,type:'settle',agencyprojectid:row.agencyprojectid,isChannel:0}))
    },
  },
  computed: {
