...
export default {
  ...
  created(){
    this.PendingSettlement(this.getData);
  },
  methods: {
    PendingSettlement(data){
      getSaleList(data).then(res=>{
        if(res.data.success){
          this.pendingList = res.data.salelist;
    ...
    goRouter(path,row){
      ...
      sessionStorage.setItem('isFalg',JSON.stringify({saleid:row.saleid,type:'pending_settlement',agencyprojectid:row.agencyprojectid,isChannel:0}))
    },
  },
  computed: {
