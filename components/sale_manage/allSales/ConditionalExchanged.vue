  ...
  methods: {
    Exchanged(data){
      getSaleList(data).then(res=>{
        if(res.data.success){
          this.unconditionalList = res.data.salelist;
    ...
    goRouter(path,row){
      ...
      sessionStorage.setItem('isFalg',JSON.stringify({saleid: row.saleid, type: 'unconditional_exchange', agencyprojectid: row.agencyprojectid, isChannel: 0}))
    },
  },
  computed: {
