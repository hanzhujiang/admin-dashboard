<template>
  <div class="wrapper clearfix ">
    ...
    <div class="wrapper-header clearfix">
      <div class="title">
        <span >{{rs.await_approving_channel}}</span>
      </div>
    </div>
          ...
          <div class="card-box">
            ...
            <div class="card-body clearfix">
              <div class="fl user-img">
                <img class="head_portrait" :src="d.photo" alt="">
              </div>
              <div class="card-content fl">
                <p class="agencyname">{{d.agencyname}}</p>
                <p>{{d.projectname}}</p>
                <p>Lot#: {{d.lotnumber}} Unit#: {{d.unitnumber}}</p>
                <p>Request: {{d.agencyname}}</p>
              </div>
        ...
        <el-row >
          <!-- <el-col :xs="24" :sm="24" :md="16" :lg="16" :xl="16">
            <div class="filter-count">
              <span>{{rs.approved_requests}}
              ( {{rs.lasted}} 20 {{rs.requests}} )</span>
            </div>
          </el-col> -->
          <el-col :xs="24" :sm="24" :md="4" :lg="4" :xl="4">
            <div class="filter-count">
              <span>{{rs.all_approvals}}({{rs.channel}})</span>
            </div>
          </el-col>
           <el-col :xs="24" :sm="24" :md="7" :lg="7" :xl="7">
             <div class="filter-count">
              <el-checkbox v-model="sign" class="filter-text" true-label="0"	false-label="1"	>{{rs.show_only_my_approvals}}({{rs.channel}})</el-checkbox>
             </div>
          </el-col>
          ...
          <el-col :xs="24" :sm="24" :md="5" :lg="5" :xl="5">
            <div class="filter-search ">
              <el-input :placeholder="rs.search_requests"  v-model="getData.condition" size="mini" 
                  class="input-key" clearable @change="search" @keyup.enter='search'>
                    ...
                    <el-table
                      ...
                      <el-table-column
                        ...
                        <template slot-scope="scope">
                          ...
                          <p style="padding-left:5px">{{scope.row.lastname}}</p>
...
<script>
import { getAgencyApprove,getAgencyApproved} from '@/api/saleApi.js'
...
export default {
  ...
  methods:{
    ...
    getRequest0(data){
      getAgencyApprove(data).then(res=>{
        if(res.data.success){
...
    },
    getRequest1(data,type){
      ...
      getAgencyApproved(submitData).then(res=>{
        if(res.data.success){
    ...
    goRouter(path,row,flag){
      ...
      sessionStorage.setItem('isFalg',JSON.stringify({saleid:row.saleid,type:goRouterType,agencyprojectid:row.agencyprojectid,requestid:row.requestid,isChannel:1}))
