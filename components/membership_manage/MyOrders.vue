    ...
    <div class="wrapper-content clearfix">
      <el-row :gutter="20">
        <el-col :span="24">
          <div class="box">
            <div class="box-body">
              <p style="color:#666;font-size:14px;padding-bottom:5px">{{rs.stripe_info}}</p>
                     ...
                     <div class="table_box">
                      ...
                      <el-table-column
                       ...
                       <el-table-column
                        ...
                        <template slot-scope="scope">
                          <div>
                           {{scope.row.synchstatus=='1'?'已支付':scope.row.synchstatus=='2'?'支付失败':scope.row.synchstatus=='3'?'即将到期':scope.row.synchstatus=='4'?'已过期':'数据不存在'}}
                          </div>
                        </template>
                      </el-table-column> -->
                    </el-table>
...
import { getMyPurchaseData } from'@/api/membershipManageApi.js'
...
export default{
  ...
  methods:{
    getPurchaseList(data){
      getMyPurchaseData(data).then(res =>{
        if(res.data.success){
