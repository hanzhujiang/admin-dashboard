  ...
  <div class="wrapper clearfix">
    ...
    <div class="wrapper-header clearfix">
      <ul class="clearfix list_view_ul">
        ...
        <li :class="[active=='2'?'fl list_view':'fl active_list_views']" style="margin-left: 85px;" @click="goRouter('/membership_manage/membership/allPartnerFirms','2')">{{rs.partner_firms}}</li>
      </ul>
    </div>
...
<script>
import { getCreditCard, synchronizeInsert ,synchronizeUpdate} from '@/api/membershipManageApi.js'
import { updateNeedSubscribe } from '@/api/agencyApi.js'
...
export default{
  ...
  methods:{
    ...
    changeNeedsubscribe(row){
      ...
      updateNeedSubscribe({agencyid:row.agencyid,needsubscribe:num}).then(res=>{
        if(res.data.success){
           this.$message({
    ...
    handleSynchronize(row){
      // agency/synchronizeInsert?fullname=11&agencyid=586247762，
      // 2就调用agency/synchronizeUpdate?fullname=11&agencyid=586247762&stripeid=cus_HALZ3LubQKoi2H&email=111@qq.com&officetelephone=11111111111
    ...
    handleInsert(row){
      synchronizeInsert(row).then(res=>{
        if(res.data.success){
          this.$message({
    ...
    handleUpdate(row){
      synchronizeUpdate(row).then(res=>{
        if(res.data.success){
          this.$message({
            message: this.rs.syn_success,
            type: "success"
