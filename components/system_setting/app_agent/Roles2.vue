  ...
  <div class="wrapper clearfix ">
    ...
    <div class="wrapper-header clearfix">
      <ul class="clearfix list_view_ul">
        <li :class="[active=='1'?'fl list_view':'fl active_list_views']" @click="goRouter('/system_setting/app_agent/roles','1')">{{rs.function_view}}</li>
        <li :class="[active=='2'?'fl list_view':'fl active_list_views']" style="margin-left: 85px;" @click="goRouter('/system_setting/app_agent/roles2','2')">{{rs.user_view}}</li>
      </ul>
    </div>
...
<script>
import { getRoleList, updateRole, insertRole, deleteRole, getAgentAppUserRoleList, updateAgentAppUserRole } from'@/api/roleApi.js'
...
export default{
  ...
  methods:{
    ...
    getUserRole(data){
      getAgentAppUserRoleList(data).then(res=>{
        if(res.data.success){
    ...
    updateRoleUser(row){
        ...
        this.saveUserRoleData.agentappuserid = row.agentappuserid;
        this.dialogVisible2();
    },
    saveUserRole(row){
      updateAgentAppUserRole(this.saveUserRoleData).then(res=>{
        if(res.data.success){
          this.getUserRole(this.getUserData);
          this.$message.success(this.rs.info_msg_update_success)
        }else{
  ...
  created(){
    ...
    readonly(){
      if(this.control.agentapp_role ==='02'){
        return true
      }else{
        return false
      }
