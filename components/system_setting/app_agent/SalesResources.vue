...
export default {
  ...
  computed: {
    ...
    readonly(){
      if(this.control.agentapp_sales_resources ==='02'){
        return true
      }else{
        return false
      }
  ...
  methods:{
    ...
    changeData(data){
      this.formData.resourceid = data.resourceid;
        getDataSaleResource({resourceid:data.resourceid,systemtype:'1'}).then(res=>{
          if(res.data.data.length == 0){
            console.log('111',{resourceid:data.resourceid,systemtype:'1'});
      ...
      if (this.flag) {
        ...
        updateSaleResource(this.formData).then(res=>{
          if(res.data.success){
            ...
            // 更新salse,资源表
            this.getDataList(this.getData)
             getJSONDataSaleResource({'languagetype':this.languagetype,systemtype:'1'}).then(res=>{
              if (res.data.success) {
