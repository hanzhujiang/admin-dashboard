...
export default{
  ...
  methods:{
    ...
    saveName(){
        ...
        insertRole({systemtype:'1',rolename:JSON.stringify(this.labeData)}).then(res=>{
          if(res.data.success){
            this.getRole(this.getData)
    ...
    getFunction(){
      getRoleFunctionList({roleid:this.roleid,systemtype:'1',languagetype:this.languagetype}).then(res=>{
        if(res.data.success){
