  ...
  <div class="wrapper clearfix">
    <form autocomplete="on">
      ...
      <div class="wrapper-header clearfix">
        ...
        <div class="tools">
          <span class="btn_text" v-if="!readonly" @click="dialogVisibleShow">
        ...
        <el-row :gutter="20">
          <el-col :span="24">
            <div class="box">
              <div class="box-body">
                <div class="table_box">
                  <el-table :empty-text="rs.no_data" :data="tableData" style="width: 100%" tooltip-effect="light" size="mini">
                    <el-table-column prop="name" :show-overflow-tooltip="true" :label="rs.name">
                      <template slot-scope="scope">
                        <span>{{scope.row.firstname}} {{scope.row.lastname}}</span>
                      </template>
                    </el-table-column>
                    <el-table-column prop="rolename" :label="rs.role" :show-overflow-tooltip="true">
                    </el-table-column>
                    <el-table-column prop="email" :label="rs.email" :show-overflow-tooltip="true">
                    </el-table-column>
                    <el-table-column prop="lastlogin" :label="rs.lasted_login" :show-overflow-tooltip="true">
                      <template slot-scope="scope">
                        <div>
                    ...
                    <el-table-column prop="startdate" :label="rs.start_date">
                      <template slot-scope="scope">
                        <div>
                    ...
                    <el-table-column prop="enddate" :label="rs.end_date">
                        <template slot-scope="scope">
                        <div>
                    ...
                    <el-table-column width="350">
                      <template slot-scope="scope">
                        <span class="btn_text" @click="getSelect(scope.row)"  v-if="!readonly">{{rs.edit}}</span>
                        <span class="btn_text" @click="del(scope.row)" v-if="!readonly">{{rs.delete}}</span>
                        <span class="btn_text" @click="reset(scope.row)" v-if="!readonly">{{rs.reset_password}}</span>
                        <span class="btn_text" @click="handleSelectLicenseShow(scope.row)"  v-if="!readonly">{{rs.licenses}}</span>
                      </template>
                    </el-table-column>
                  </el-table>
                </div>
    ...
    <el-dialog title="Select Agent" :close-on-click-modal='false' :visible.sync="dialogVisible" width="60%" cka>
      <template slot="title">
      ...
      <div class="dialog-body">
        ...
        <el-row>
          <el-table :data="usersList" style="width: 100%" :empty-text="rs.no_data" size="mini">
            <el-table-column prop="name" :label="rs.name">
              <template slot-scope="scope">
                <span>{{scope.row.firstname}} {{scope.row.lastname}}</span>
              </template>
            </el-table-column>
            <el-table-column prop="phonenumber" :label="rs.phone_number">
            </el-table-column>
            <el-table-column prop="email" :label="rs.email">
            </el-table-column>
            <el-table-column prop="department" :label="rs.department">
            </el-table-column>
            <el-table-column :label="rs.action" width="100">
              <template slot-scope="scope">
      ...
      <el-row class="dailog-footer" style="margin-top:5px" v-if="popTotal">
        <el-pagination @current-change="popHandleCurrentChange" :current-page.sync="popPagesize" :page-size="popSelect" layout="prev, pager, next " :total="popTotal" class="fr">
        </el-pagination>
      </el-row>
    </el-dialog>

    <el-dialog title="Select Agent" :close-on-click-modal='false' :visible.sync="datePicker" width="50%">
      <template slot="title">
        <div class="dialog-header">
      ...
      <form autocomplete="on">
        <div class="dialog-body">
          <el-row :gutter="10">
            ...
            <el-col :xs="24" :sm="24" :md="12" :lg="12" :xl="12">
              <div class="form-group">
                ...
                <div class="content">
                  <el-date-picker :disabled="readonly" v-model="dateValue" type="date" format="yyyy-MM-dd" value-format="yyyy-MM-dd" placeholder="" size="small">
                  </el-date-picker>
                </div>
              </div>
            </el-col>
          ...
          <!-- <el-row :gutter="10">
            <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
              <div class="form-group">
                <div class="sub-title">{{rs.user_type}}</div>
                <div class="content">
                  <el-select v-model="roleid" size="small" placeholder="" :disabled="readonly">
                    <el-option v-for="(item, i) in roleList" :label="item.value" :value="item.code" :key="i">
                    </el-option>
                  </el-select>
                </div>
              </div>
      ...
      <div class="dailog-footer-tools">
        <el-row>
          <el-col :span="12">
            <div class="fl">
              <el-button   style="color:#000;" class="small" @click="close">
                {{rs.close}}
              </el-button>
            </div>
          </el-col>
          <el-col :span="12">
            <div class="fr">
              <el-button   class="small bg_blue" @click="upDataSave" v-if="!readonly">
                {{rs.save}}
              </el-button>
            </div>
          </el-col>
            ...
            <el-col :xs="24" :sm="24" :md="24" :lg="24" :xl="24">
              <el-table
                ...
                <el-table-column 
                  ...
                  width="150">
                  <template slot-scope="scope">
                    <span v-if="scope.row.isauth == '1'" class="btntext" @click="handleRelieve(scope.row)">{{rs.relieve}}</span>
                    <span v-if="scope.row.isauth == '0' && scope.row.usednumber !=scope.row.usernumber" class="btntext"  @click="handleLicenses(scope.row)">{{rs.grant_license}}</span>
                    <!-- <el-button v-if="scope.row.isauth == '1'" class="small bg_blue" size="mini" @click="handleRelieve(scope.row)">{{rs.relieve}}</el-button>
                    <el-button v-if="scope.row.isauth == '0' && scope.row.usednumber !=scope.row.usernumber" class="small bg_red" size="mini" @click="handleLicenses(scope.row)">{{rs.grant_license}}</el-button> -->
                  </template>
                </el-table-column>
              </el-table>
            </el-col>
...
import { getAppUserList, insertAppUser, deleteAppUser, updateAppUser, getUnregistAgent, resetPassword } from "@/api/agentApp.js";
import { getUserLicense, insertUserLicense, deleteUserLicense} from '@/api/userApi.js'
...
export default {
  ...
  created() {
    ...
    this.popSelect = JSON.parse(
      localStorage.getItem("selectlist")
    ).DATA_COUNT_LIST[0].intvalue;
    this.restoration();
    // this.roleList = JSON.parse(localStorage.getItem("selectlist")).USER_TYPE_LIST;
    // this.roleList.forEach(val => {
    //   this.userObj[val.code] = val.value;
    // });
  },
  components: {
  ...
  methods: {
    ...
    getSelect(row) {
      ...
      getRoleList({systemtype:'1',languagetype:localStorage.getItem('languagetype')}).then(res=>{
        if (res.data.success) {
          if (this.roleid==0) {
            this.roleid =  ""
          }
    ...
    del(row) {
      this.$confirm(this.rs.info_msg_delete_question, this.rs.prompt_message, {
        ...
        .then(() => {
          deleteAppUser({ ids: row.userid }).then(res => {
            if (res.data.success) {
              this.getData("0");
              this.$message({
                type: "success",
    ...
    arrpopList(data) {
      getUnregistAgent(data).then(res => {
        if (res.data.success) {
     ...
     handleRelieve(row){
      ...
      center: true
        ...
        deleteUserLicense({ids:row.userlicenseid}).then(res =>{
            if(res.data.success){
              this.$message.success(this.rs.info_msg_update_success);
              this.getUserLicenseList({userid:row.userid,systemtype:'01',isactive:'1'});
            }else{
              this.$message.error({dangerouslyUseHTMLString:true,message:this.ErrorMsgStr(res.data.data)});
            }
          })
    ...
    // 授权
    handleLicenses(row){
      insertUserLicense({userid:row.userid,licenseid:row.licenseid,roleid:row.roleid,usertype:'1'}).then(res =>{
        if(res.data.success){
          this.$message.success(this.rs.info_msg_update_success);
          this.getUserLicenseList({userid:row.userid,systemtype:'01',isactive:'1'});
        }else{
  ...
  computed: {
    ...
    readonly() {
      if (this.control.agentapp_user === "02") {
        return true;
      } else {
        return false;
      }
  ...
  watch: {
    ...
    se(val){
      ...
      this.roleList = val.USER_TYPE_LIST
      this.roleList.forEach(val => {
        this.userObj[val.code] = val.value;
      });
    }
