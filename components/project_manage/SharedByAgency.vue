                ...
                <div class="card_box" @click="goRouter('/project_manage/projectDetail',d.projectid,d,'')">
                  ...
                  <div class="porject_name">
                   ...
                   <div class="agency_name">
                    <!-- <img src="" alt=""> -->
                    {{d.agencynum}}&nbsp;{{d.agencynum == '1'?rs.agency:rs.agencies}}
                    <!-- {{d.accepted}} -->
                  </div>
                  <div class="actionBtn">
                    <span class="btntext fr" @click.stop="goRouter('/project_manage/edit_project/shareTo',d.projectid,d,'',true)">{{rs.view_shared_list}}</span>
                    <!-- <span class="btntext fr" @click="downloadUploadingShow(scope.row)" v-if="!readonly">{{rs.agreement}}</span> -->
                  </div>
...
<script>
import { getProjectPictureData, getSharedToAgency} from'@/api/projectApi.js'
...
export default{
  ...
  components: {
    ...
    this.getSharedToAgencyList(this.data)
  ...
  methods:{
    ...
    goRouter(name,id,obj,index,type){
      if (obj) {
       sessionStorage.setItem('ProjectList',JSON.stringify(obj));
       if(type){
        sessionStorage.setItem('TabName','shareTo')
       }
      }
      if(id){
    ...
    //所有楼盘信息
    getSharedToAgencyList(projectList){
      getSharedToAgency(projectList).then(res=>{
        if (res.data.success) {
