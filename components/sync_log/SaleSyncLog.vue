                ...
                <el-table
                  ...
                  <el-table-column
                    ...
                    <span :class="scope.row.status==0?'yellow':''">
                      {{ scope.row.operatetype == 1 ? 'Trigger New Sale' :  'Trigger Updated Sale' }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column
                    :label="rs.status">
...
<script>
import { getSaleSyncData, getSaleOneSyncData } from '@/api/syncApi.js'
...
export default{
  ...
  methods:{
    ...
    getSyncData(data){
      ...
      getSaleOneSyncData(param).then(res=>{
        if(res.data.success){
