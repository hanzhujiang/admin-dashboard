              ...
              <div class="table_box">
                <el-table
                  ...
                  <el-table-column
                    :label="rs.company">
                    <template slot-scope="scope">
                      <span :class="scope.row.status==0?'yellow':''">{{ scope.row.ownername }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column
                  ...
                  <el-table-column
                    ...
                    <span :class="scope.row.status==0?'yellow':''">
                      {{ scope.row.operatetype == 1 ? 'Trigger New Client' : 
                          scope.row.operatetype == 2 ?'Trigger Updated Client' : 'Action Upsert' }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column
                    :label="rs.status">
...
<script>
import { getClientSyncData, getClientOneSyncData } from '@/api/syncApi.js'
...
export default{
  ...
  methods:{
    ...
    getSyncData(data){
      ...
      getClientOneSyncData(param).then(res=>{
        if(res.data.success){
    ...
    getSyncList(data){
      getClientSyncData(data).then(res=>{
        if(res.data.success){
