                ...
                <el-table
                  ...
                  <el-table-column
                    ...
                    <span :class="scope.row.status==0?'yellow':''">
                      {{ scope.row.operatetype == 2 ? 'Trigger Updated Property' : 'Action Update' }}</span>
                    </template>
                  </el-table-column>
                  <el-table-column
                    :label="rs.status">
...
<script>
import { getPropertySyncData, getPropertyOneSyncData } from '@/api/syncApi.js'
...
export default{
  ...
  methods:{
    ...
    getSyncData(data){
      ...
      getPropertyOneSyncData(param).then(res=>{
        if(res.data.success){
    ...
    getSyncList(data){
      getPropertySyncData(data).then(res=>{
        if(res.data.success){
